...
import org.boon.slumberdb.handlers.AdminHeartbeatOld;
import org.boon.slumberdb.handlers.AdminHeartbeat;
...
public class ResponseUtil extends Verticle {
    ...
    DataStoreServer dataStoreServer = new DataStoreServer() {
        ...
        protected void sendToAllClients(String reply) {
            ResponseUtil.this.sendToAllClients(reply);
        }
        ...
        protected void sendMessageToClientId(String clientId, String reply) {
            ...
            ResponseUtil.this.sendMessageToClientId(clientId, reply);
        }
        ...
        protected void registerOutputHandler(String clientId, Object commChannel) {
            ...
            ResponseUtil.this.registerOutputHandler(clientId, commChannel);
        }
    };
    ...
    private Logger logger = configurableLogger(ResponseUtil.class);
    ...
    private RouteMatcher adminRouteMatchers(Map<String, ClassMeta<?>> servicesDefinition) {
        ...
        AdminHeartbeatOld adminHeartbeatOld = new AdminHeartbeatOld(config, dataStoreServer.dataStoreConfig());
        AdminHeartbeat adminHeartbeat = new AdminHeartbeat(config, dataStoreServer.dataStoreConfig());

        AdminSearchUsers adminSearchUsers = new AdminSearchUsers(dataStoreServer);

        routeMatcher.get("/admin/heartbeat", adminHeartbeatOld);
        routeMatcher.get("/admin/heartbeat/", adminHeartbeatOld);

        routeMatcher.get("/admin/heartbeat2", adminHeartbeat);
        routeMatcher.get("/admin/heartbeat2/", adminHeartbeat);

        routeMatcher.get("/admin/search/users", adminSearchUsers);
        routeMatcher.get("/admin/search/users/", adminSearchUsers);
    ...
    private void handleRESTCall(final String restURI, final HttpServerRequest request) {
        ...
        if (request.path().equals(restURI)) {
            ...
            if (request.method().equals("GET")) {
                ...
                int status = setOptionHeaders(request) ? 200 : 400;
    ...
    private boolean setOptionHeaders(HttpServerRequest request) {
